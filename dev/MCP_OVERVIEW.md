...
Lifecycle is driven by `vscode-mcp.core` via `mcp/fxs.cljs` and `mcp/server.cljs` (`build-lifecycle-config`). Manual stop and deactivate always unregister from Cursor (best-effort), then stop the socket; deactivate is silent. Manual stop sets `:lifecycle/needs-cursor-reregister?` for the next start to force client reload.
...
**ECA registration** (independent of Cursor): when `:mcp/auto-register-eca?` is on and extension `editor-code-assistant.eca` is installed, the lifecycle hook upserts project-local `.eca/config.json` (server key from `:cursor/server-name`, managed `command`/`args` only; portable args via `:lifecycle/wrapper-install-dir` → wrapper at `${env:HOME}/.config/calva/backseat-driver/calva-mcp-server.js` installed by vscode-mcp on MCP start — DEBUG symlink / release copy, library port file under `~/.config/vscode-mcp/port-files/`, configured host). Requires a workspace folder; no deregister on stop; no command or when-contexts.
